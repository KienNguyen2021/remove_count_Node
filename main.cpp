...
public:
	...
	void removeNode(Staff staff)   // pass the argument staff that we want to remove
	{
		...
		if (pPrevious != nullptr)     // means : the node to be removed not at the top of the list
		{
			...
			if (pPrevious->pNext == nullptr || pPrevious->pNext->datum.salary != staff.salary)
			...
			else     // means : the node to be remobed at the top of the list, so the pPrevious = null pointer from H node
			{
				...
				if (pHead == nullptr || pHead->datum.salary != staff.salary)
					return;
...
int main()
{
	...
	list1.printLinkedList();
	cout << endl;
	cout << " Number of members of the list BEFORE Remove :" << list1.numberOfMembers() << endl;

	cout << endl;

	cout <<list1.isMember(staff2) << endl;    // 1 :  it is in the list 


	list1.removeNode(staff2);   // want to remove staff2 (Paul 19)

	list1.printLinkedList();

	cout << endl;
	cout << " Number of members of the list AFTER Remove :" << list1.numberOfMembers() << endl;

	cout << endl;
	cout << list1.isMember(staff2) << endl;    // 0 : after remove Paul 19
